...
export const like = functions.region(REGION).https.onRequest(
  async (req: Request, res: Response) => {
    try {
      // Step 0: Validate request authentication (Firebase Auth)
      const auth = await validateRequestAuthentication(req, res);
      const user_guid = auth.uid;

      // Step 1: Extract required fields from body
      const { comment_guid, method } = req.body;

      // Step 2: Check for missing fields
      if (!comment_guid || !method) {
